...
def create_dependency(param_groupid,param_artifactid,param_version,param_classifier):  
...
def merge_dependency(pom_et,param_artifactid,param_groupid,param_version,param_classifier):
  artifactfound=False  
  ...
  for dependency_element in pom_et.findall('pom:dependencies/pom:dependency',pom_ns):  
    ...
    if (checkgroupid == param_groupid and checkartifactid == param_artifactid and (checkclassifier == param_classifier or param_classifier is None)):
      artifactfound=True  
  ...
  if not artifactfound:  
    ...
    dependency_element = create_dependency(param_groupid,param_artifactid,param_version,param_classifier)
...
for line in lines:
	...
	if len(temp)>2:
		...
		artifactid=temp[1]
		version=temp[2]
                file=pomlocation+"/pom.xml"
                #read the file at the pomlocation parameter  
		pom_et = get_tree_from_xmlfile(file)  
   		#merge the dependency into the obtained ElementTree  
		pom_et,pom_et_changed=merge_dependency(pom_et,artifactid,groupid,version,classifier)  
     		#overwrite the pomlocation if it has been changed   
		if pom_et_changed:  
  			print ("Overwriting "+file+" with changes" ) 
  			target = open(file, 'w')  
  			target.truncate()  
  			target.write(elementtree_to_str(pom_et))  
  			target.close()  
		else:  
  			print (file+" does not require changes"  )
